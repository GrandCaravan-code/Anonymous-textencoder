...
ИЗМЕНЕНО: Добавлена логика приоритета NER над Regex.
ИЗМЕНЕНО: Основная функция anonymize_text_file сделана асинхронной.
ИЗМЕНЕНО: Модифицирована merge_and_filter_results для приоритета Stanza NER.
ИЗМЕНЕНО: Исправлена ошибка инициализации StanzaRecognizer.
ИЗМЕНЕНО: Добавлено детальное логирование списков результатов на каждом этапе.
ИЗМЕНЕНО: Переписана merge_and_filter_results на двухпроходный метод с "якорями".
ИЗМЕНЕНО: Блокирующие вызовы NLP (analyze, run_natasha_ner, anonymize) вынесены в потоки с помощью asyncio.to_thread.
ИЗМЕНЕНО: Расширен NER_FALSE_POSITIVE_FILTER.
ИЗМЕНЕНО: Добавлена функция _adjust_ner_scores для понижения score подозрительных NER-результатов (строчная буква).
ИЗМЕНЕНО: Модифицирована логика финальной фильтрации NER (шаг 8) для более гибкой проверки NER_FALSE_POSITIVE_FILTER.
ИЗМЕНЕНО: Глобальные пороги и множители вынесены в config.py.
"""
import logging
import stanza
import spacy
